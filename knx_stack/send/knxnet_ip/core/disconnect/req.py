...
Msg = namedtuple('DisconnectRequest', ['addr_control_endpoint', 'port_control_endpoint'])
...
def send(state, msg):
    ...
    >>> disconnect_request = Msg(addr_control_endpoint='127.0.0.1', port_control_endpoint=1234)
    >>> (final_state, final_msg) = send(state, disconnect_request)
    >>> final_msg
