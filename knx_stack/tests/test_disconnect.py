...
class TestDisconnect(unittest.TestCase):
    ...
    def test_disconnect_request_decode(self):
        ...
        msg = knx_stack.knxnet_ip.Msg.make_from_str(self.DISCONNECT_REQUEST_MSG)
        data = knx_stack.decode_msg(state, msg)
        self.assertEqual(len(data), 1)
        self.assertIsInstance(data[0], knx_stack.knxnet_ip.core.disconnect.req.Msg)
    ...
    def test_disconnect_response_encode(self):
        ...
        self.assertEqual(str(encoded).upper(), self.DISCONNECT_RESPONSE_MSG.upper())
    ...
    def test_disconnect_request_encode(self):
        ...
        disconnect_req = knx_stack.knxnet_ip.core.disconnect.req.Msg(
            addr_control_endpoint='127.0.0.1',
            port_control_endpoint=1234
        )
        encoded = knx_stack.encode_msg(state, disconnect_req)
        self.assertEqual(str(encoded).upper(), self.DISCONNECT_REQUEST_MSG.upper())
