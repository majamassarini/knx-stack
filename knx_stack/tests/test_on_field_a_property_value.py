...
if __name__ == "__main__":
    address_table = knx_stack.layer.AddressTable(0x102D, [], 255)
    ...
    for i in (0x66, 0x67, 0x68):
        msgs.append((knx_stack.send.layer.application.a_property_value_read.req.Msg(asap=0, object_index=0, property_id=i,
    ...
    msgs.append(
        knx_stack.send.layer.application.a_group_value_read.req.Msg(asap=1))
    msgs.append(
        knx_stack.send.layer.application.a_group_value_read.req.Msg(asap=2))
    msgs.append(
        knx_stack.send.layer.application.a_group_value_read.req.Msg(asap=3))
    msgs.append(
        knx_stack.send.layer.application.a_property_value_write.req.Msg(asap=0, object_index=0, property_id=0x79,
                                                                    number_of_elements=1, start_index=0x1F, data='02'))  # potenza attiva
    msgs.append(
        knx_stack.send.layer.application.a_property_value_read.req.Msg(asap=0, object_index=0, property_id=0x79,
    ...
    #                                  start_index=0x21))
    msgs.append(
        knx_stack.send.layer.application.a_property_value_write.req.Msg(asap=0, object_index=0, property_id=0x7A,
                                                                    number_of_elements=1, start_index=0x1F, data='02'))
    msgs.append(
        knx_stack.send.layer.application.a_property_value_read.req.Msg(asap=0, object_index=0, property_id=0x7A,
                                                                   number_of_elements=1, start_index=0x1F))
    ...
    #                                  start_index=0x21))
    msgs.append(
        knx_stack.send.layer.application.a_property_value_write.req.Msg(asap=0, object_index=0, property_id=0x7B,
                                                                    number_of_elements=1, start_index=0x1F, data='02'))
    msgs.append(
        knx_stack.send.layer.application.a_property_value_read.req.Msg(asap=0, object_index=0, property_id=0x7B,
                                                                   number_of_elements=1, start_index=0x1F))
    ...
    for msg in msgs:
        if isinstance(msg, knx_stack.send.layer.application.a_property_value_write.req.Msg):
            (_, final_msg) = knx_stack.send.layer.application.a_property_value_write.req.send(state, msg)
        elif isinstance(msg, knx_stack.send.layer.application.a_property_value_read.req.Msg):
            (_, final_msg) = knx_stack.send.layer.application.a_property_value_read.req.send(state, msg)
        else:
            (_, final_msg) = knx_stack.send.layer.application.a_group_value_read.req.send(state, msg)
