...
def receive(state, msg):
    ...
    if tsap:
        if state.ldata.apci == 0:
            (data, state) = application.a_group_value_read.ind.receive(state, msg)
        elif (state.ldata.apci - state.ldata.data) == 0x40:
            (data, state) = application.a_group_value_response.ind.receive(state, msg)
        elif (state.ldata.apci - state.ldata.data) == 0x80:
            (data, state) = application.a_group_value_write.ind.receive(state, msg)
    else:
